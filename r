    ...
    if MutationsData then
        ...
        if success and mutationModule then
            ...
            if mutationInfo and mutationInfo.Palettes then
                -- Pick a random palette (like the game does)
                local randomPaletteIndex = math.random(1, #mutationInfo.Palettes)
                local palette = mutationInfo.Palettes[randomPaletteIndex]
                
                if palette then
                    -- Get all BaseParts first
                    local parts = {}
                    ...
                    -- Apply colors evenly distributed across parts
                    for i, part in ipairs(parts) do
                        local colorIndex = ((i - 1) % #palette) + 1
                        part.Color = palette[colorIndex]
                        
                        -- Special handling for Cyber mutation (semi-transparent + glowy)
