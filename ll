...
local function getTraitsFromOverhead(overhead)
    ...
    if container then
        for _, icon in ipairs(container:GetChildren()) do
            if icon:IsA("ImageLabel") and icon.Image ~= "" then
                local assetId = icon.Image
                if traitMap[assetId] then
                    table.insert(traits, traitMap[assetId].name)
                else
                    print("[VisualDupe] Unknown trait icon:", assetId)
                end
            end
        end
    end
    print("[VisualDupe] Detected traits:", table.concat(traits, ", "))
    return traits
end
    ...
    if mutLabel and mutLabel.Visible then
        ...
        for name, data in pairs(mutationMap) do
            if text:find(name) or text:find(data.displayText) then
                print("[VisualDupe] Detected mutation:", name)
                return name
            end
        end
        print("[VisualDupe] Unknown mutation text:", text)
    end
    return "None"
end
...
local function spawnAnimal(animalName, selectedTraits, selectedMutation)
    ...
    -- DUAL PROXIMITY PROMPTS: E = Grab (top), F = Sell (bottom)
    if clonedAnimal.PrimaryPart then
        -- Grab prompt part (higher up)
        local grabPart = Instance.new("Part")
        grabPart.Name = "GrabPromptPart"
        grabPart.Transparency = 1
        grabPart.Anchored = true
        grabPart.CanCollide = false
        grabPart.CanQuery = true
        grabPart.Size = Vector3.new(1, 1, 1)
        grabPart.CFrame = clonedAnimal.PrimaryPart.CFrame + Vector3.new(0, 2, 0)
        grabPart.Parent = clonedAnimal
        ...
        sellPart.CFrame = clonedAnimal.PrimaryPart.CFrame + Vector3.new(0, 0.5, 0)
        sellPart.Parent = clonedAnimal

        local sellPrompt = Instance.new("ProximityPrompt")
        sellPrompt.ObjectText = animalData.DisplayName
